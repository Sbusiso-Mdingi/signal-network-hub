    ...
    <footer className="border-t border-border">
      <div className="mx-auto grid max-w-6xl gap-10 px-6 py-14 md:grid-cols-3">
        ...
        <div className="grid gap-8 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2">
          <nav aria-label="Explore" className="flex flex-col gap-3 text-sm">
            ...
            <Link to="/for-medical-schemes" className="text-muted-foreground hover:text-foreground">
              For Medical Schemes
            </Link>
          </nav>
          <nav aria-label="Company" className="flex flex-col gap-3 text-sm">
            ...
            <Link to="/governance" className="text-muted-foreground hover:text-foreground">
              Governance
            </Link>
            <Link to="/about" className="text-muted-foreground hover:text-foreground">
              About
            </Link>
            <a href={APP_SIGN_IN_URL} className="text-muted-foreground hover:text-foreground">
              Sign In
            </a>
          </nav>
        </div>
      ...
      <div className="border-t border-border">
        <p className="mx-auto max-w-6xl px-6 py-6 text-xs text-muted-foreground">
          © {new Date().getFullYear()} Sequrin. All rights reserved.
        </p>
      </div>
    </footer>
  );
